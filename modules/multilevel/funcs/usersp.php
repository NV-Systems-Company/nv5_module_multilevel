...
{
    $num_count = $db->query('SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE userid=' . $userid)->fetchColumn();
    if (  $num_count  == 1 ){
        $result = $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET status=1, edit_time=' . NV_CURRENTTIME . ' WHERE userid =' . $userid );
...
$sql = 'SELECT t1.lev, t1.datatext, t2.userid, t2.username, t2.first_name, t2.last_name, t2.birthday, t2.email FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users AS t1 INNER JOIN ' . NV_USERS_GLOBALTABLE . ' AS t2 ON t1.userid=t2.userid WHERE t1.status != 1 AND t1.istype = 1 AND parentid = ' . $user_info['userid'] . ' ORDER BY t1.sort ASC';
