...
define('NV_IS_TABLE_SHOPS',$db_config['prefix'] . '_shops');
$page_title = $lang_module['reward'];
...
$sql = 'SELECT t1.*, t2.title, t2.mobile, t2.email, t2.adminid FROM ' . $table_name . ' AS t1 INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_customer AS t2 ON t1.user_id=t2.cid WHERE ' . $where;
$query = $db->query($sql);
...
if( !empty( $array_orderid )){
    $sql = 'SELECT t2.order_id, t2.num, t2.price, t1.id, t1.product_price FROM ' . NV_IS_TABLE_SHOPS . '_rows AS t1 INNER JOIN ' . NV_IS_TABLE_SHOPS . '_orders_id AS t2 ON t1.id=t2.proid WHERE t2.order_id IN(' . implode(',', $array_orderid ) . ')';
    $query = $db->query($sql);

    $array_data_reward = array( );
    while ($row = $query->fetch()) {
        $price_sale = $row['price'] - $row['gia_von'];
        $row['reward'] = $price_sale * $row['num'];
        if( isset( $array_data_reward[$row['order_id']] )){
            $array_data_reward[$row['order_id']] += $row['reward'];
        }else{
            $array_data_reward[$row['order_id']] = $row['reward'];
        }
    }
...
foreach ( $array_data as $data ){
    $data['reward'] = $array_data_reward[$data['order_id']];
    $data['reward'] = number_format( $data['reward'], 0, '.', ',');
