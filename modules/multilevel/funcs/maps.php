...
if(defined('NV_IS_USER')){
	...
	if( $nv_Request->isset_request( 'setactive', 'post' )){
		...
		if( $user_data_affiliate['permission'] == 1 ){
			$sql = 'SELECT status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE status=0 AND userid = ' . $userid;

			$result = $db->query( $sql );
			list( $active ) = $result->fetch(3);

			if ($active == 0) {
				$sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET status=1 WHERE userid=' . $userid;
				$result = $db->query($sql);
				$sql = 'UPDATE ' . NV_USERS_GLOBALTABLE . ' SET active=1 WHERE userid=' . $userid;
				$result = $db->query($sql);
				$db->query('UPDATE ' . NV_PREFIXLANG . '_regsite SET status=1 WHERE userid=' . $userid );
		...
		if( $user_data_affiliate['permission'] == 1 || $data_users['parentid'] == $user_info['userid'] ){
			if (!empty($data_users)  ) {
				...
				$sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET pendingdelete=' . $time_pendingdelete . ' WHERE userid=' . $data_users['userid'];
				$db->query($sql);

				$array_reponsive = array('status' => 1, 'message' => $lang_module['message_reponsive_delete_2'] );
				/*
				$sql = 'DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE userid = ' . $userid;
				if ($db->exec($sql)) {
		...
		$sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE userid=' . $userid . ' AND pendingdelete> 0';
		$data_users = $db->query($sql)->fetch();

		if( !empty($data_users) && $user_data_affiliate['permission'] == 1 || $data_users['parentid'] == $user_info['userid'] ){
			$sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET pendingdelete=0 WHERE userid=' . $data_users['userid'];
			$db->query($sql);
	...
	$sql = 'SELECT t1.*, t2.numsubcat, t2.code, t2.possitonid, t2.lev, t2.agencyid, t2.provinceid, t2.status, t2.pendingdelete FROM ' . NV_USERS_GLOBALTABLE . ' AS t1 INNER JOIN ' . NV_PREFIXLANG . '_' . $module_data . '_users AS t2 ON t1.userid=t2.userid WHERE t2.ishidden=0 AND t1.userid =' . $user_info['userid'];
	$res = $db->query( $sql );
