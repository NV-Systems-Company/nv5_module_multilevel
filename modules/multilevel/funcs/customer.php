...
if(defined('NV_IS_USER')){
	if( $nv_Request->isset_request( 'delete_customer_id', 'get' ) and $nv_Request->isset_request( 'delete_checkss', 'get' ) )
	{
		...
		{
			$db->query( 'DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users  WHERE customer_id = ' . $db->quote( $customer_id ) );
	...
	if( $nv_Request->isset_request( 'submit', 'post' ) )
	{
		...
		$sql = "SELECT customer_id FROM " . NV_PREFIXLANG . '_' . $module_data . "_users WHERE phone LIKE '%" . $row['phone'] . "%' AND fullname LIKE '%" . $row['fullname'] . "%' AND customer_id !=" . $row['customer_id'];
		...
		if( empty( $error ) )
		{
			try
			{
				...
				if( empty( $row['customer_id'] ) )
				{
					...
					$sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_users  (refer_userid, code, fullname, address, phone, email, description, add_time, edit_time, custype, status) VALUES (:refer_userid, :code, :fullname, :address, :phone, :email, :description, :add_time, :edit_time, :custype, :status)';
				...
				{
					$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET refer_userid=:refer_userid, code=:code, fullname = :fullname, address = :address, phone = :phone, email = :email, description = :description, status = :status, custype=:custype, edit_time=:edit_time WHERE customer_id=' . $row['customer_id'] );
	...
	{
		$row = $db->query( 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE customer_id=' . $row['customer_id'] )->fetch();
		if( empty( $row ) )
		{
			Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op );
			die();
	...
	if( ! $nv_Request->isset_request( 'customer_id', 'post,get' ) && ! $nv_Request->isset_request( 'add', 'post,get' ) )
	{
		...
		$sql = ' SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users';

		$db->sqlreset()->select( 'COUNT(*)' )->from( '' . NV_PREFIXLANG . '_' . $module_data . '_users' );
		$sql_where = ' parentid = ' . $user_info['userid'];
		if( ! empty( $q ) )
		{
		...
		$db->select( '*' )->order( 'userid DESC' )->limit( $per_page )->offset( ( $page - 1 ) * $per_page );
		$sth = $db->prepare( $db->sql() );
	...
	if( $show_view )
	{
		...
		{ 
			...
			$view['custype'] = $lang_module['custype_' . $view['istype']];
