...
if( $nv_Request->isset_request( 'setactive', 'post' )){
    ...
    $sql = 'SELECT status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE userid = ' . $userid;
    $result = $db->query( $sql );
    ...
    $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET status=' . $active . ' WHERE userid=' . $userid;
...
elseif( $nv_Request->isset_request( 'permission', 'post' )){
    ...
    $sql = 'SELECT permission FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE userid = ' . $userid;
    $result = $db->query( $sql );
    list( $active ) = $result->fetch(3);
    ...
    $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET permission=' . $active . ' WHERE userid=' . $userid;
    ...
    if (md5($id . NV_CHECK_SESSION) == $checkss) {
        ...
        if ($db->exec($sql)) {
            $db->query( 'DELETE FROM ' . NV_PREFIXLANG . '_regsite WHERE userid = ' . $id );
            ...
            if( $data_users['parentid'] > 0 ){
                ...
                $sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE userid=' . $data_users['parentid'];
                ...
                $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET numsubcat=numsubcat-1, subcatid=' . $db->quote( $data_users_parent['subcatid'] ) . ' WHERE userid=' . $data_users['parentid'];
                $db->query($sql);
...
$savecat = 0;
$array_data = array();
list($array_data['userid'], $array_data['parentid'], $array_data['possitonid'], $array_data['agencyid'], $array_data['cityid'], $array_data['districtid'] ) = array(0,0,0,0,0,0,0);
...
if ($userid > 0 ) {
    ...
    $sql = 'SELECT t1.userid, t1.parentid, t1.weight, t1.numsubcat, t1.mobile, t1.datatext, t1.possitonid, t1.agencyid, t1.cityid, t1.districtid, t2.username, t2.first_name, t2.last_name, t2.birthday, t2.email FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users AS t1 INNER JOIN ' . NV_USERS_GLOBALTABLE . ' AS t2 ON t1.userid=t2.userid WHERE t1.userid = ' . $userid;
    $result = $db->query( $sql );
...
if (! empty($savecat)) {
    ...
    $array_data['cityid'] = $nv_Request->get_int('cityid', 'post', 0);
    ...
    list( $userid, $haveorder, $parentid ) = $db->query('SELECT userid, haveorder, parentid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE userid=' . $array_data['userid'] )->fetch( 3 );
    ...
    if( empty( $error )){
        try{
            if ( $array_data['userid'] > 0 && $userid == 0 ) {
                ...
                $weight = $db->query('SELECT max(weight) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users WHERE parentid=' . $array_data['parentid'])->fetchColumn();
                $weight = intval($weight) + 1;
                $listparentid = $subcatid = '';

                $stmt = $db->prepare("INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_users (userid, parentid, precode, code, mobile, salary_day, benefit, datatext, weight, sort, lev, possitonid, agencyid, numsubcat, subcatid, listparentid, add_time, edit_time, status, cityid, districtid, permission, haveorder, shareholder) VALUES
			(:userid, :parentid, :precode, :code, :mobile, :salary_day, :benefit, :datatext, :weight, '0', '0', :possitonid, :agencyid, '0', :subcatid, :listparentid, " . NV_CURRENTTIME . ", " . NV_CURRENTTIME . ", 1, :cityid, :districtid, 0, 0, 0)");
                ...
                $stmt->bindParam(':cityid', $array_data['cityid'], PDO::PARAM_INT);
                ...
                $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_users SET parentid=' . $array_data['parentid'] . ', mobile=:mobile, salary_day=:salary_day, benefit=:benefit, datatext=:datatext, possitonid=:possitonid, agencyid=:agencyid, edit_time=' . NV_CURRENTTIME . ', cityid=:cityid, districtid=:districtid WHERE userid =' . $array_data['userid']);
...
$sql = 'SELECT t1.lev, t1.possitonid, t1.agencyid, t1.mobile, t2.userid, t2.username, t2.first_name, t2.last_name, t2.birthday, t2.email FROM ' . NV_PREFIXLANG . '_' . $module_data . '_users AS t1 INNER JOIN ' . NV_USERS_GLOBALTABLE . ' AS t2 ON t1.userid=t2.userid ORDER BY t1.sort ASC';
...
$array_city = $nv_Cache->db( 'SELECT * FROM ' . NV_PREFIXLANG . "_" . $module_data . '_city WHERE status=1 ORDER BY weight', 'id', 'location' );
foreach( $array_city as $city )
{
    $xtpl->assign( 'OPTION', array(
        'key' => $city['id'],
        'title' => $city['title'],
        'selected' => ( $city['id'] == $array_data['cityid'] ) ? ' selected="selected"' : '' ) );
    $xtpl->parse( 'main.content.select_city' );
}
if( $array_data['cityid'] > 0 ){
