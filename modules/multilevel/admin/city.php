...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		foreach( $del_array as $city_id )
		{
			...
			if( $ward_total = $db_slave->query( 'SELECT COUNT(*) total FROM ' . TABLE_LOCALION_NAME . '_ward WHERE city_id = ' . ( int )$city_id )->fetchColumn() )
			{
				$json['error'] = sprintf( $lang_module['city_error_ward'], $ward_total );
			}
			elseif( $district_total = $db_slave->query( 'SELECT COUNT(*) total FROM ' . TABLE_LOCALION_NAME . '_district WHERE city_id = ' . ( int )$city_id )->fetchColumn() )
			{
				$json['error'] = sprintf( $lang_module['city_error_district'], $district_total );
			}
			else
			{
				$db_slave->query( 'DELETE FROM ' . TABLE_LOCALION_NAME . '_city WHERE city_id = ' . ( int )$city_id );
...
if( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $data['city_id'] > 0 )
	{
		$result = $db_slave->query( 'SELECT * FROM ' . TABLE_LOCALION_NAME . '_city WHERE city_id=' . $data['city_id'] );
		$data = $result->fetch();
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		if( empty( $error ) )
		{
			...
			if( $data['city_id'] == 0 )
			{
				...
				$weight = $db_slave->query( 'SELECT max(weight) FROM ' . TABLE_LOCALION_NAME . '_city' )->fetchColumn();
				$weight = intval( $weight ) + 1;

				$query = 'INSERT INTO ' . TABLE_LOCALION_NAME . '_city(title,alias,weight,status,area,important) VALUES(   
