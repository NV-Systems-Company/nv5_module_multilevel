...
if( ACTION_METHOD == 'weight' )
{
	...
	{
		$json['error'] = $lang_module['ward_error_security'];
	}
	...
	if( ! isset( $json['error'] ) )
	{
		if( $new_vid > 0 )
		{
			...
			$json['success'] = $lang_module['ward_update_success'];
		}
	}
...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		foreach( $del_array as $ward_id )
		{
			...
			$db_slave->query( 'DELETE FROM ' . TABLE_LOCALION_NAME . '_ward WHERE ward_id = ' . ( int )$ward_id );
		...
		if( $count )
		{
			...
			$json['success'] = $lang_module['ward_delete_success'];
		}
		else
		{
			$json['error'] = $lang_module['ward_error_delete'];
		}
...
if( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $data['ward_id'] > 0 )
	{
		$data = $db_slave->query( 'SELECT * FROM ' . TABLE_LOCALION_NAME . '_ward WHERE ward_id=' . $data['ward_id'] )->fetch();
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		if( empty( $error ) )
		{
			...
			if( $data['ward_id'] == 0 )
			{
				...
				$weight = $db_slave->query( 'SELECT max(weight) FROM ' . TABLE_LOCALION_NAME . '_ward' )->fetchColumn();
				...
				$data['ward_id'] = ( int )$db_slave->insert_id( $query );
